...
export class VaultcoreClient {
    ...
    async postObject(key, data, contentType) {
        try {
            const url = encodeURIComponent(key).replace(/%2F/g, '/');
            const headers = {};
            let body;
...
class BaseCommand extends Command {
    ...
    getClient() {
        const baseUrl = this.baseUrl || process.env.ENV_REPO_BASE_URL || process.env.VAULTCORE_BASE_URL || '';
...
class CreateCommand extends BaseCommand {
    ...
    async execute() {
        try {
            ...
            const res = await client.postObject(this.key, arrayBuffer, this.contentType).catch(errorHandler);
...
class UpdateCommand extends BaseCommand {
    ...
    async execute() {
        try {
            ...
            const buffer = await fs.readFile(this.filePath);
            const arrayBuffer = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
            const res = await client.putObject(this.key, arrayBuffer, this.contentType).catch(errorHandler);
            this.context.stdout.write(`${res.key}\n`);
            return 0;
        }
        catch (error) {
